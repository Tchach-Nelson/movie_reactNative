...
const HomeScreen = () => {
    ...
    handleSearch = async () => {
        ...
        if(texte == ''){
            ...
            let result = await ApiServices(`${ApiConfig.baseUrl}/search/movie?include_adult=false&language=fr&page=1&query=${texte}`, 'GET', '', ApiConfig.apiToken);
            setTerme(texte)
            setSearch(result);
    
            if (result.status == 200) {
                setShowSearch(true) ;
                setSearch(result.response);
            }
            else {
                ...
                <View style={{padding: 20, background:theme.colors.background}}>
                    ...
                    <View style={{marginTop: 10, display:'flex', flexDirection:"row", alignItems:"center", justifyContent:"center"}}>
                        <View style={{marginTop: 15, display:'flex', flexDirection:"row" , justifyContent:"center", alignItems: 'center' ,  width: "90%", borderRadius: 100, borderColor: 'gray', borderWidth: .5,}}>
                            ...
                            />
                             <TouchableOpacity onPress={this.handleSearch} style={{backgroundColor:'#00000018',display: 'flex', alignItems:'center', justifyContent: 'center', width: 45, height: 45, borderRadius:100, marginLeft: 50}}>
